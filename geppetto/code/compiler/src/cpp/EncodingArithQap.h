...
class LEncodedEltQapArith;
class REncodedEltQapArith;

class EncodedEltQapArith : public EncodedElt {

};


class EncodedProductQapArith;

class CondensedEncEltQapArith : public CondensedEncElt { 
};

// For fields with elements that consist of msft digit_t
class EncodedEltQapArithArray : public EncodedEltArray {
public:
	...
	EncodedEltQapArithArray(elt_t t, Encoding* encoding, uint32_t len, bool allocate);
	virtual ~EncodedEltQapArithArray();
...
private:
	uint32_t len;
	EncodedEltQapArith** elts;  
	bool allocated;	// Did we allocate the elts?
};


class EncodingArithQap : public EncodingExp {
public:
	EncodingArithQap();
	~EncodingArithQap();	
  ...
  virtual uint32_t size_of_prod();
	...
	virtual void print_raw(elt_t t, EncodedElt* e);
...
protected:		
	EncodingArithQapMembers* members;
